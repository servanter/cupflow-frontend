<template>
  <view class="page">
    <view class="header">
      <text class="header-tip">{{ isFirstTime ? '完善以下信息，开始体验 CupFlow' : '修改你的个人资料' }}</text>
    </view>
    ...
    <view class="section">
      ...
      <!-- #ifndef MP-WEIXIN -->
      <view class="avatar-btn" @tap="onChooseImageH5">
        <view class="avatar-wrap">
          <image v-if="displayUrl" :src="displayUrl" class="avatar-img" mode="aspectFill" />
          <view v-else class="avatar-placeholder">
            <text class="avatar-placeholder-icon">📷</text>
    ...
    <view class="submit-area">
      ...
      <text v-if="isFirstTime" class="skip-text" @tap="handleSkip">跳过，稍后再设置</text>
    </view>
  </view>
</template>
...
// H5端：uni.chooseImage 选择图片
const onChooseImageH5 = () => {
  uni.chooseImage({
    count: 1,
    sizeType: ["compressed"],
...
const uploadAvatar = async (tempFilePath: string) => {
  tempDisplayUrl.value = tempFilePath;
